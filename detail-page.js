...
if (detailTarget) {
  ...
  const journeyCards = (pageJourneys[key] || [["Explore", "All RACO expertise", "/expertise/"], ["Choose a model", "How we work", "/engagement-models/"], ["Begin free", "Book a discovery session", "/contact/"]]).map((item) => `<a href="${item[2]}"><span>${item[0]}</span><h3>${item[1]} ↗</h3></a>`).join("");
